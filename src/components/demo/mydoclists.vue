      ...
      <v-data-table :headers="headers" :items="desserts" sort-by="projectUser" class="elevation-1">
        <template v-slot:top>
          <v-toolbar flat color="white">
            <v-toolbar-title>我创建的全部文件</v-toolbar-title>
        ...
        <template v-slot:no-data>
          <v-btn color="primary" @click="initialize">打开我的文件列表</v-btn>
        </template>
      </v-data-table>
    </div>
  </div>
  ...
  methods: {
    ...
    initialize() {
      this.$axios({
        ...
        if (res.data.errno === 0) {
          this.$message.success("获取我的文件列表成功");
          this.desserts = res.data.my_file_list
