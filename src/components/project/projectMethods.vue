...
export default {
  created(){
    ...
    // this.delete_project_recycle_bin(1)
    // this.cancel_delete_project(1)
    // this.showTeam()
    // this.get_all_project_list()
  },
  methods:{
    //收藏功能,通过测试
    star_project(ID){
  ...
  delete_project_recycle_bin(ID){
    ...
    cancel_delete_project (ID){
      ...
      this.$axios({
        method: 'get',
        url: '/api/project/cancle_delete_project',
        // data: qs.stringify({
        //   projectID:ID,
        //   teamID: teamID
        //   // teamID: 1
    ...
    showTeam(){
      this.$axios({
        method: 'post',
        url: '/api/team/show_my_team_list',
      })
        .then(res => {
          console.log(res.data)
          if (res.data.errno === 0) {
            this.$message.success("获取项目信息成功");
          } else {
            this.$message.error(res.data.msg);
          }
        })
